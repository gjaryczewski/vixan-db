...
BEGIN TRY
    ...
    IF NOT EXISTS (SELECT * FROM dbo.Threads WHERE Id = @ThreadId AND [Status] = 'STARTED')
        THROW 50003, 'The thread with the given Id is not started.', 1;

    DECLARE @ProcessId int = (SELECT TOP(1) ProcessId FROM dbo.Threads WHERE Id = @ThreadId);
    ...
    INSERT dbo.OperationLog (OperationId, [Status])
        VALUES (@OperationId, 'STARTED');

    DECLARE @ScriptName nvarchar(128) = (SELECT TOP(1) ScriptName FROM dbo.Operations WHERE Id = @OperationId);
